...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "Something went wrong generating refresh and access tokens"
    );
  }
};
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    throw new ApiError(400, "user not found or exist");
  }

  const isPasswordValid = user.isPasswordCorrect(password);
  if (!isPasswordValid) {
    throw new ApiError(401, "invalid credential password is not correct");
  }
